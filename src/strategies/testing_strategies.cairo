...
mod Strategy {
    ...
    use cairo_arena::constants::{GRID_WIDTH, GRID_HEIGHT};
    ...
    impl Strategy of super::IStrategy<ContractState> {
        fn determin_action(
            ...
            if my_character.position.y < GRID_HEIGHT - 1 {
                let new_grid = my_character.position.x + (my_character.position.y + 1) * GRID_HEIGHT;
            ...
            if direction == Direction::Right {
                if my_character.position.x < GRID_WIDTH - 1 {
                    let new_grid = my_character.position.x + 1 + my_character.position.y * GRID_HEIGHT;
                    if arenaGrid.get(new_grid.into()) != 0 {
